...
function RegisterTraj_Noise_Pan( pg1, pg2, pan_dyna)
    global ctx;
    ctx = [];
...
function r = residual_noJ( x )
    ...
    Tpanmoveable_cam2 = Cart2T_ros(x(7:12));
    scaleFactor = x(13);
    totalT = Tbase_panbase*ctx.Tpanbase_movable*Tpanmoveable_cam2;
    ...
    T_unwrap = inv(T_rpy_bar)*inv(T_rpy_pan)*inv(T_rpy_base);
    T_unwrap = eye(4);
    ...
    for ii = 1:n
        ...
        total_res = abs([ctx.traj_c1(1:3,ii); 1] - Tbase_panbase*ctx.Tpanbase_movable*Tpanmoveable_cam2*[ctx.traj_c2(1:3,ii).*scaleFactor;1]);
        %s_xyz = S(1:3,1:3);
        r = [r; total_res];
    end
    ctx.iter =  ctx.iter + 1;
...
function plot_system( ctx )
    ...
    Tpanmoveable_cam2 = Cart2T_ros(ctx.x(7:12));
